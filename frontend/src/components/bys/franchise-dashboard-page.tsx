...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { className: string }> = {
    ...
    CONFIRMED: { className: 'bg-[#FFD54F]/10 text-[#FFD54F] border-[#FFD54F]/20' },
    IN_PROGRESS: { className: 'bg-[#FFD54F]/5 text-[#FFD54F] border-[#FFD54F]/20' },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#0A1F44] sm:text-3xl">Franchise Dashboard</h1>
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-[#F2C94C]">Franchise Owner</span>
            </div>
            <h2 className="mt-1 text-2xl font-bold text-white">
              {user?.name?.split(' ')[0] || 'Franchise'} 👋
            </h2>
            <p className="mt-1 text-[#F2C94C]/80">Your franchise at a glance</p>
          </div>
        </div>
      </motion.div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-2xl font-bold text-[#132D5E]">
                  ...
                  {recentBookings.map((booking) => (
                    ...
                    >
                      ...
                      <div className="flex shrink-0 flex-col items-end gap-1">
                        ...
                        <span className="text-xs font-semibold text-[#FFD54F]">₹{booking.amount.toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div className="rounded-xl bg-[#FFD54F]/5 p-4 text-center">
                  ...
                  <p className="mt-1 text-lg font-bold text-[#0A1F44]">{user?.city || 'Not Assigned'}</p>
                </div>
