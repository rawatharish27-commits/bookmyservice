      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card className="bg-white rounded-xl">
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2"><Mail className="size-4 text-blue-600" /><CardTitle className="text-sm font-semibold text-slate-900">Templates</CardTitle></div>
          </CardHeader>
          <CardContent className="space-y-0">
            {templates.map((t, i) => (
              <div key={t.id}>
                <div className="flex items-center gap-4 py-3">
                  <div className="flex size-9 items-center justify-center rounded-lg bg-blue-50"><Mail className="size-4 text-blue-600" /></div>
        ...
        <Card className="bg-white rounded-xl">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {variables.map((v) => (
                <button key={v} className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-mono text-slate-600 hover:bg-blue-100 hover:text-blue-600 transition-colors"><Copy className="size-3 inline mr-1" />{v}</button>
              ))}
            </div>
          </CardContent>
        </Card>
