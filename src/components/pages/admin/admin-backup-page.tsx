    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6">
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" className="gap-1 bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white rounded-xl"><Plus className="size-4" /> Create Backup</Button>
        </div>

        <div className="grid grid-cols-3 gap-4">
          <Card className="bg-white rounded-xl"><CardContent className="p-4 text-center"><Database className="size-5 text-[#0A1F44] mx-auto mb-1" /><p className="text-lg font-bold text-slate-900">245 MB</p><p className="text-xs text-slate-500">Latest Backup Size</p></CardContent></Card>
            ...
            {backups.map((backup, i) => (
              <div key={backup.id}>
                <div className="flex items-center gap-4 py-3">
                  <div className="flex size-10 items-center justify-center rounded-lg bg-[#FFD54F]/10"><HardDrive className="size-5 text-[#0A1F44]" /></div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2"><p className="text-sm font-medium text-slate-900">{backup.name}</p><Badge variant="secondary" className={backup.type === 'Auto' ? 'bg-[#FFD54F]/10 text-[#0A1F44] border-[#FFD54F]/20' : 'bg-purple-100 text-purple-700 border-purple-200'}>{backup.type}</Badge></div>
