              ...
              <div className="flex gap-2 flex-wrap">
                {starFilters.map(star => (
                  <button key={star} onClick={() => setFilterStar(filterStar === star ? null : star)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterStar === star ? 'bg-[#0A1F44] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {star} ★
                  </button>
                ))}
              </div>
