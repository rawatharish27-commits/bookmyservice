            ...
            {conversations.map((conv, i) => (
              <div key={conv.id}>
                <button onClick={() => setSelectedChat(conv.id)} className={`w-full flex items-center gap-3 p-4 hover:bg-slate-50 transition-colors text-left ${selectedChat === conv.id ? 'bg-blue-50' : ''}`}>
                  <div className="flex size-10 items-center justify-center rounded-full bg-blue-100 text-sm font-bold text-blue-600">{conv.name.charAt(0)}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {conv.unread > 0 && <span className="flex size-4 items-center justify-center rounded-full bg-blue-600 text-[10px] text-white">{conv.unread}</span>}
                    </div>
                  </div>
                </button>
        ...
        {selectedChat && (
          <div className={`flex-1 flex flex-col bg-white ${!selectedChat ? 'hidden sm:flex' : ''}`}>
            <div className="flex items-center gap-3 p-4 border-b border-slate-200">
              ...
              <div className="flex size-8 items-center justify-center rounded-full bg-blue-100 text-sm font-bold text-blue-600">R</div>
            ...
            <ScrollArea className="flex-1 p-4 space-y-3">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === 'provider' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] rounded-xl px-3 py-2 ${msg.sender === 'provider' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-800'}`}>
            ...
            <div className="p-4 border-t border-slate-200 flex gap-2">
              ...
              <Button className="bg-blue-600 hover:bg-blue-700 gap-1"><Send className="size-4" /></Button>
            </div>
          </div>
        )}
      </div>
