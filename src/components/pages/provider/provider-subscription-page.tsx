      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {plans.map((plan) => {
            ...
            return (
              <Card key={plan.name} className={`bg-white rounded-xl relative ${plan.popular ? 'border-2 border-[#0A1F44]' : ''}`}>
                {plan.popular && <Badge className="absolute -top-2 left-1/2 -translate-x-1/2 bg-[#0A1F44] text-white border-0">Most Popular</Badge>}
                <CardContent className="p-5">
                  ...
                  <Button className={`w-full mt-4 rounded-xl ${plan.current ? 'bg-slate-100 text-slate-500' : plan.name === 'Pro' ? 'bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white' : plan.name === 'Premium' ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-slate-200 text-slate-700'}`} disabled={plan.current}>
