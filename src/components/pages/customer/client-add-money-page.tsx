  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <Card className="bg-gradient-to-r from-[#0A1F44] to-[#0A1F44] rounded-xl border-0 text-white">
          <CardContent className="p-5">
            <p className="text-xs text-[#FFD54F]/80">Current Balance</p>
            ...
            <div className="grid grid-cols-3 gap-3">
              {presets.map((p) => (
                <Button key={p} variant={amount === String(p) ? 'default' : 'outline'}
                  ...
                  className={`rounded-xl py-6 text-lg font-bold ${amount === String(p) ? 'bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white' : 'border-slate-200 text-slate-900'}`}
          ...
          <CardContent className="space-y-2">
            {methods.map((m) => (
              <button key={m.id} onClick={() => setSelectedMethod(m.id)}
                className={`flex w-full items-center gap-3 rounded-xl border p-3 transition-colors ${selectedMethod === m.id ? 'border-[#0A1F44] bg-[#FFD54F]/10' : 'border-slate-100 hover:bg-slate-50'}`}
                aria-pressed={selectedMethod === m.id}>
                <m.icon className={`size-5 ${selectedMethod === m.id ? 'text-[#0A1F44]' : 'text-slate-400'}`} />
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium text-slate-900">{m.label}</p>
                  <p className="text-xs text-slate-400">{m.desc}</p>
                </div>
                {selectedMethod === m.id && <Check className="size-5 text-[#0A1F44]" />}
              </button>
            ))}
          </CardContent>
        </Card>

        <Button className="w-full bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white gap-1 rounded-xl py-5" disabled={!amount || submitting} onClick={handleAddMoney} aria-label={`Add ₹${amount || '0'} to wallet`}>
