      ...
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <Card className="bg-white rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              <div className="flex size-10 items-center justify-center rounded-lg bg-[#FFD54F]/10"><MessageSquare className="size-5 text-[#0A1F44]" /></div>
        ...
        <Card className="bg-white rounded-xl">
          <CardContent className="p-4 space-y-3 max-h-80 overflow-y-auto">
            {detail.messages.map((m) => (
              <div key={m.id} className={`flex ${m.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-xl p-3 ${m.from === 'user' ? 'bg-[#0A1F44] text-white' : 'bg-slate-100 text-slate-700'}`}>
          ...
          <div className="p-3 flex gap-2">
            ...
            <input id="support-msg" type="text" placeholder="Type a message..." className="flex-1 rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-[#0A1F44]/20" />
