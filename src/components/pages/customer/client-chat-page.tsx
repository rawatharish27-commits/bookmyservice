    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col">
      <div className="bg-white border-b border-slate-200 p-4">
        <div className="mx-auto max-w-2xl flex items-center gap-3">
          <Avatar><AvatarFallback className="bg-[#0A1F44] text-white text-sm">{chatData.providerInitials}</AvatarFallback></Avatar>
        ...
        <div className="mx-auto max-w-2xl space-y-3">
          {chatData.messages.map((m) => (
            <div key={m.id} className={`flex ${m.from === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl p-3 ${m.from === 'user' ? 'bg-[#0A1F44] text-white rounded-br-md' : 'bg-white text-slate-700 rounded-bl-md shadow-sm border border-slate-100'}`}>
      ...
      <div className="bg-white border-t border-slate-200 p-3">
        <div className="mx-auto max-w-2xl flex gap-2">
          ...
          <input id="chat-msg" type="text" placeholder="Type a message..." className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-[#0A1F44]/20" />
          <Button size="icon" className="shrink-0 bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white rounded-xl" aria-label="Send message"><Send className="size-4" /></Button>
        </div>
      </div>
    </div>
  )
