          ...
          <CardContent className="space-y-4">
            {[
              ...
              <div key={item.key} className="flex items-center justify-between">
                ...
                <button onClick={() => toggle(item.key)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${currentSettings[item.key] ? 'bg-blue-600' : 'bg-slate-200'}`} role="switch" aria-checked={currentSettings[item.key]} aria-label={`${item.label}: ${currentSettings[item.key] ? 'on' : 'off'}`}>
