            ...
            {amcData.plans.map((plan) => (
              <Card key={plan.id} className={`rounded-xl ${plan.color}`}>
                <CardContent className="p-5 space-y-3">
                  ...
                  <p className="text-xl font-bold text-[#0A1F44]">{plan.price}</p>
                  ...
                  <Button className="w-full gap-1 bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white rounded-xl text-xs" aria-label={`Buy ${plan.name} plan`}><Zap className="size-3" />Buy Now</Button>
                </CardContent>
              </Card>
            ))}
          </div>
