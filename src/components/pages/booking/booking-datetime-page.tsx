  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <Card className="bg-white rounded-xl">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-semibold text-slate-900"><Calendar className="size-4 inline mr-1.5 text-[#0A1F44]" />May 2025</CardTitle>
            ...
            <div className="grid grid-cols-7 gap-2">
              {dates.map((d) => (
                <button key={d.date} disabled={!d.available} onClick={() => d.available && setSelectedDate(d.date)}
                  className={`flex flex-col items-center gap-1 rounded-xl p-2 transition-colors ${!d.available ? 'opacity-30 cursor-not-allowed' : selectedDate === d.date ? 'bg-[#0A1F44] text-white' : 'hover:bg-slate-50'}`}>
        ...
        <Card className="bg-white rounded-xl">
          <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-slate-900"><Clock className="size-4 inline mr-1.5 text-[#0A1F44]" />Available Time Slots</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-cols-3 gap-2">
              {timeSlots.map((slot) => (
                <Button key={slot.time} disabled={!slot.available}
                  ...
                  className={`rounded-xl text-xs ${selectedTime === slot.time ? 'bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white' : !slot.available ? 'opacity-30' : 'border-slate-200'}`}>
                  {slot.time}
                </Button>
              ))}
            </div>
        ...
        <Button className="w-full bg-[#0A1F44] hover:bg-[#0A1F44]/90 text-white gap-1 rounded-xl py-5">Confirm Date & Time</Button>
      </div>
    </div>
  )
}
