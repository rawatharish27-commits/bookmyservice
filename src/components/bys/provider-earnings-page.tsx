  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { label: 'Total Earnings', value: `₹${totalEarnings.toLocaleString()}`, icon: DollarSign, gradient: '[#0A1F44] to-[#132D5E]', bgGlow: 'bg-[#0A1F44]/10' },
          { label: 'Pending Payout', value: `₹${pendingPayout.toLocaleString()}`, icon: Clock, gradient: '[#0A1F44] to-[#D4A017]', bgGlow: 'bg-[#0A1F44]/10' },
          { label: 'This Month', value: `₹${thisMonthEarnings.toLocaleString()}`, icon: TrendingUp, gradient: '[#0A1F44] to-[#132D5E]', bgGlow: 'bg-[#0A1F44]/10' },
        ].map((stat, idx) => (
          <motion.div
            key={stat.label}
            ...
            {completed.length === 0 ? (
              <div className="p-8 text-center">
                <div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-[#FFD54F]/10">
                  <CheckCircle2 className="size-8 text-[#0A1F44]/40" />
                </div>
              ...
              <ScrollArea className="max-h-96">
                {completed.map((booking) => (
                  <div key={booking.id} className="flex items-center justify-between border-b p-4 last:border-0 transition-colors hover:bg-[#FFD54F]/10/30">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-[#0A1F44] flex items-center gap-1">
