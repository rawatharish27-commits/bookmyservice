...
export const onRequestPatch: PagesFunction<Env> = async (context) => {
  try {
    ...
    if (auth.role !== 'PROVIDER' && auth.role !== 'ADMIN') {
      return error('Only providers can reject bookings', 403);
    }
    ...
    if (booking.status !== 'PENDING') return error('Booking is not in PENDING status', 400);
    ...
    await execute(context.env.DB,
      'UPDATE Booking SET status = \'REJECTED\', cancellationReason = ?, updatedAt = datetime("now") WHERE id = ?',
      [body.reason || null, id]
    );

    return json({ message: 'Booking rejected', status: 'REJECTED' });
  } catch (e) {
