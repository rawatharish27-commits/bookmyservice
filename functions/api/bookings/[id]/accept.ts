...
export const onRequestPatch: PagesFunction<Env> = async (context) => {
  try {
    ...
    if (auth.role !== 'PROVIDER' && auth.role !== 'ADMIN') {
      return error('Only providers can accept bookings', 403);
    }
    ...
    await execute(context.env.DB,
      'UPDATE Booking SET status = \'ACCEPTED\', updatedAt = datetime("now") WHERE id = ?',
      [id]
    );

    return json({ message: 'Booking accepted', status: 'ACCEPTED' });
  } catch (e) {
