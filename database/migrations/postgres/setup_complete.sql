-- ============================================================================
-- BookYourService — Complete PostgreSQL Setup for Supabase
-- ============================================================================
-- This file combines the schema (0001_init.sql) and seed data (0002_seed.sql)
-- into a single script for easy one-time setup in the Supabase SQL Editor.
--
-- Conversion notes (SQLite D1 → PostgreSQL):
--   • INTEGER PRIMARY KEY AUTOINCREMENT → SERIAL PRIMARY KEY
--   • REAL → DOUBLE PRECISION
--   • INTEGER used for booleans → BOOLEAN (DEFAULT true/false)
--   • datetime('now') → NOW()
--   • TEXT DEFAULT (datetime('now')) → TIMESTAMPTZ DEFAULT NOW()
--   • scheduledDate TEXT → DATE, scheduledTime TEXT → TIME
--   • effectiveDate TEXT → DATE
--   • All table/column names quoted with double quotes for camelCase safety
--   • IF NOT EXISTS retained on all CREATE TABLE and CREATE INDEX
--   • Sequence values reset after explicit ID inserts
--   • Admin password hash regenerated as bcrypt (from PBKDF2-SHA512)
--   • Privacy policy updated to reflect bcrypt hashing
-- ============================================================================
