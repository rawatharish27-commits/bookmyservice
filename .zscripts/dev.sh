...
cleanup() {
        if [ -n "${DEV_PID:-}" ] && kill -0 "$DEV_PID" >/dev/null 2>&1; then
                echo "Stopping Vite dev server (PID: $DEV_PID)..."
...
log_step_start "bun install (root)"
echo "[BUN] Installing root dependencies..."
bun install
log_step_end "bun install (root)"

log_step_start "bun install (frontend)"
echo "[BUN] Installing frontend dependencies..."
cd "$PROJECT_DIR/frontend" && bun install
log_step_end "bun install (frontend)"

log_step_start "Starting Vite dev server"
echo "[BUN] Starting frontend development server..."
cd "$PROJECT_DIR"
bun run dev &
DEV_PID=$!
...
echo "Vite dev server is running in background (PID: $DEV_PID)."
